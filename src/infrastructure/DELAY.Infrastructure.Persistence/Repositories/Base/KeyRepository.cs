...
{
    internal class KeyRepository<TEntity, TDomain> : BaseRepository<TEntity, TDomain>, IKeyStorage<TDomain>
        where TDomain : class, IKey
        where TEntity : class, IKey
    {
        protected readonly IModelMapperService _mapper;

        protected KeyRepository(DelayContext context, IModelMapperService mapper) : base(context)
        {
            this._mapper = mapper;
        }
